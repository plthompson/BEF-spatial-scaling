...
S2b<-ggplot(slopes.summary,aes(x=Scale,y=Slopes))+
...
for(i in 1:1000){
  ...
  L_SR<-sapply(1:2, FUN = function(x){(L_SR+x)}) #diversity change
...
S2a<-ggplot(slopes.summary2,aes(x=Scale,y=slope_mean))+
  geom_ribbon(aes(ymin=lower,ymax=upper),alpha=0.3,color=NA)+
  geom_line()+
  theme_bw()+
  removeGrid()+
...
S3b<-ggplot(inequality.df,aes(x=BEF_slope,y=Function_change))+
  geom_line()+
  theme_bw()+
  removeGrid()+
...
S3a<-ggplot(inequality.df2,aes(x=Local_SR,y=Function_change))+
  geom_line()+
  theme_bw()+
  removeGrid()+
